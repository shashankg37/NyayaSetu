...
try:
    ...
    # File is at backend/app/ai/config.py, so parent.parent.parent gives backend/
    backend_dir = Path(__file__).resolve().parent.parent.parent
    root_env = backend_dir.parent / ".env"  # Go up one more to repo root
    if root_env.exists():
...
ROOT_DIR = Path(__file__).resolve().parent.parent.parent  # backend/ folder
DATA_DIR = ROOT_DIR / "data"
KB_DIR = DATA_DIR / "knowledge_base"
QDRANT_DIR = DATA_DIR / "qdrant"
TEMPLATES_DIR = ROOT_DIR / "app" / "ai" / "raw_templates" / "drafts"
